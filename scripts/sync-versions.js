...
if (versions.size > 1) {
	console.error("\nERROR: Not all release packages have the same version.");
...
console.log("\nAll release packages are at the same version (lockstep).");
...
for (const pkg of workspacePackages) {
	for (const dependencyType of ["dependencies", "devDependencies"]) {
		...
		for (const [dependencyName, currentSpecifier] of Object.entries(dependencies)) {
			// Registry aliases and local path/workspace links are never workspace-versioned specs.
			if (/^(file:|link:|workspace:|npm:)/.test(currentSpecifier)) {
				continue;
			}
			const version = versionMap.get(dependencyName);
			const newSpecifier = version
				? currentSpecifier.startsWith("^")
					? `^${version}`
					: version
				: null;
