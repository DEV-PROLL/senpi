 ...
 *
 * Source manifests stay on local lockstep workspace versions so local builds and
 * tests resolve the current workspace packages. Release tooling owns publish-only
 * dependency rewrites.
 */
...
const INDEPENDENT_VERSION_PACKAGES = new Set(["@earendil-works/pi-storage-sqlite-node"]);
...
const publishedPackages = workspacePackages.filter(
	(pkg) => pkg.data.private !== true && !INDEPENDENT_VERSION_PACKAGES.has(pkg.data.name),
);
...
function nextWorkspaceSpecifier(currentSpecifier, version) {
	if (
		currentSpecifier.startsWith("file:") ||
		currentSpecifier.startsWith("workspace:") ||
		currentSpecifier.startsWith("npm:") ||
		currentSpecifier === "*"
	) {
		return null;
	}
	return currentSpecifier.startsWith("^") ? `^${version}` : version;
}
...
for (const pkg of workspacePackages) {
	for (const dependencyType of ["dependencies", "devDependencies"]) {
		...
		for (const [dependencyName, currentSpecifier] of Object.entries(dependencies)) {
			...
			const newSpecifier = version ? nextWorkspaceSpecifier(currentSpecifier, version) : null;
