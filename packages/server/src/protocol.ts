...
type ExactKeys<T, Keys extends PropertyKey> = keyof T extends Keys ? true : false;
...
type _AiModelInputsAccountedFor = Assert<AiModelInput extends ProtocolModelInput | "video" ? true : false>;
type _ProtocolModelInputsFitAi = Assert<ProtocolModelInput extends AiModelInput ? true : false>;
type AiAssistantContentType = AssistantMessage["content"][number]["type"];
type _AiAssistantContentTypesAccountedFor = Assert<
	AiAssistantContentType extends "text" | "thinking" | "toolCall" | "providerNative" ? true : false
>;
/**
...
type _AiThinkingContentFieldsAccountedFor = Assert<
	ExactKeys<
		...
		"type" | "thinking" | "startedAt" | "endedAt" | "thinkingSignature" | "redacted"
	>
>;
...
function toProtocolModelInputs(inputs: readonly AiModelInput[]): ProtocolModelInput[] {
	return inputs.filter((input): input is ProtocolModelInput => input === "text" || input === "image");
}
...
export function toProtocolModelMetadata(model: Model<Api>, authenticated: boolean): ModelMetadata {
	const result = {
		...
		input: toProtocolModelInputs(model.input),
...
function toProtocolAssistantContent(message: AssistantMessage): AssistantTranscriptItem["content"] {
	return message.content.flatMap((part): AssistantTranscriptItem["content"] => {
		switch (part.type) {
			...
			default: {
				const contentType = (part as { type: string }).type;
				if (contentType === "providerNative") return [];
				throw new TypeError(`Unsupported assistant content type: ${contentType}`);
			}
		}
	});
}
