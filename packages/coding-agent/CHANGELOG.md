...
- Fixed system-owned TTSR interruptions being recorded as user interruptions, which could block an active Goal even
  ...
  work before agent settlement even when it arrives after TTSR or extension dispatch. A terminal system error with
  no retry or monitor now queues a guarded Goal-owned recovery instead of leaving an active Goal idle. Stream-rule
