...
- Hooks trust-state reads keep complete snapshots on a lock-free fast path while malformed or empty reads now acquire the bounded writer lock and revalidate under writer exclusion. This prevents mixed-version legacy writers from hiding an absent-active-absent lock cycle around truncate-and-rewrite; a still-malformed exclusive reread or exhausted live writer lock fails closed without surfacing `ELOCKED`. Serialized current writers continue to publish complete snapshots through same-directory atomic rename.
