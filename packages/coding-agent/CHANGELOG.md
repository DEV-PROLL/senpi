...
- Ambient Claude SDK OAuth authentication now preserves request-scoped token overrides through stored and ambient auth replay, isolates request tokens from host token slots and subprocess-control variables, remains valid across resident and auxiliary calls, recognizes request tokens in explicit ambient mode, and shares bounded availability probes without abandoned-request ownership ([#836](https://github.com/code-yeongyu/senpi/pull/836)).
