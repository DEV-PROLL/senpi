...
function openSession(entries: Entry[]): SessionManager {
	const directory = join(
		tmpdir(),
		`upstream-model-id-restore-${Date.now()}-${Math.random().toString(36).slice(2)}`,
	);
	temporaryDirectories.push(directory);
	mkdirSync(directory, { recursive: true });
	const file = join(directory, "session.jsonl");
	const header = { type: "session", version: 3, id: "upstream-model-id-restore", timestamp: "2026-09-04T00:00:00.000Z", cwd: directory };
