...
function account(
  overrides: Partial<CursorCliAccountSlot> = {},
...
describe("Cursor CLI OAuth per-account HOME store", () => {
  it("creates a private directory tree and writes the file-store credential at mode 0600", async () => {
    ...
    const completed = await runInCursorAccountHome(
      agentDir,
      slot,
      async ({ home, authPath }) => {
        expect(home).toBe(
          join(agentDir, "cursor-cli-oauth", "accounts", slot.name, "home"),
        );
  ...
  it("keeps the HOME durable across runs while reapplying auth before every spawn", async () => {
    ...
    const first = await runInCursorAccountHome(
      agentDir,
      original,
      async ({ authPath, home }) => {
        ...
        writeFileSync(
          cliConfigPath,
          '{"permissions":{"allow":["first-cli-rewrite"]}}',
        );
        writeFileSync(
          authPath,
          JSON.stringify({
    ...
    const second = await runInCursorAccountHome(
      agentDir,
      first.slot,
      async ({ authPath }) => {
        expect(readFileSync(cliConfigPath, "utf8")).toContain(
          "first-cli-rewrite",
        );
        ...
        writeFileSync(
          cliConfigPath,
          '{"permissions":{"allow":["second-cli-rewrite"]}}',
        );
        writeFileSync(
          authPath,
          JSON.stringify({
  ...
  it("rejects a malformed CLI credential instead of treating file existence as refresh success", async () => {
    ...
    await expect(
      runInCursorAccountHome(agentDir, account(), async ({ authPath }) => {
        writeFileSync(
          authPath,
          JSON.stringify({ refreshToken: { token: "misleading" } }),
        );
      }),
