...
} from "../src/core/extensions/builtin/claude-agent-sdk/custom-tools.ts";
import { createToolWatch, registerToolWatch } from "../src/core/extensions/builtin/claude-agent-sdk/tool-watch.ts";
import {
	BUILTIN_SDK_TOOLS,
	canUseTool,
	mapPiToolNameToSdk,
...
} from "../src/core/extensions/builtin/claude-agent-sdk/tools.ts";
...
describe("Claude Agent SDK tool integration", () => {
	...
	it("executes an SDK-proposed Read through senpi's tool contract", async () => {
		const cwd = await mkdtemp(join(tmpdir(), "claude-agent-sdk-tools-"));
		try {
		...
		watch.hydrate(sessionKey, [
			{
				...
				customType: "claude-agent-sdk-tool-watch",
