...
} from "../../src/core/extensions/builtin/claude-agent-sdk/account-command.ts";
import {
	type AccountSlot,
	addAccount,
...
import { selectAccount } from "../../src/core/extensions/builtin/claude-agent-sdk/affinity.ts";
import type { ClaudeAgentSdkProviderSettings } from "../../src/core/extensions/builtin/claude-agent-sdk/settings.ts";
...
function credential(...accounts: AccountSlot[]): ClaudeAgentSdkCredential {
...
describe("/claude-account", () => {
	...
	it("persists a pin that deterministically overrides HRW selection", async () => {
		...
		const stored = storage.get(PROVIDER_ID) as ClaudeAgentSdkCredential;
		...
		expect(notices.at(-1)).toEqual({ message: "Pinned Claude Agent SDK account: bravo.", type: "info" });
	});
	...
	it("reports an unknown pin without changing the stored account state", async () => {
		...
		expect(notices.at(-1)).toEqual({
			message: "Claude Agent SDK account 'missing' does not exist.",
			type: "error",
		});
	});
});
