...
- The inherited supervisor pipe is the primary watchdog signal on Win32: both `end` and `close` trigger teardown, while the slower identity fallback requires three consecutive missing probes so a timed-out PowerShell query cannot delay or spuriously trigger lifecycle cleanup.
