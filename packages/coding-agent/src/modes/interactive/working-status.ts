...
export function largeSessionWorkingStatusInterval(sessionEntryCount: number, defaultIntervalMs: number): number {
	return sessionEntryCount >= LARGE_SESSION_ENTRY_THRESHOLD
		? LARGE_SESSION_WORKING_STATUS_INTERVAL_MS
		: defaultIntervalMs;
}
