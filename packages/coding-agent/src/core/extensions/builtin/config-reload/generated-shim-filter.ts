...
const GENERATED_SHIM_BANNER_PREFIX = "// Generated by senpi.";
...
function hasGeneratedShimBanner(path: string): boolean {
	try {
		return readFileSync(path, "utf-8").startsWith(GENERATED_SHIM_BANNER_PREFIX);
	} catch {
		return false;
	}
}
...
export function excludeGeneratedExtensionShims(paths: readonly string[], agentDir: string): string[] {
	return paths.filter((path) => !(isInsideExtensionsDirectory(path, agentDir) && hasGeneratedShimBanner(path)));
}
