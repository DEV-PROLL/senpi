...
export const DEFAULT_AFFINITY_KEY = "claude-agent-sdk-default";
	...
	constructor(soonestUnblockAt: number | undefined) {
		super(
			soonestUnblockAt === undefined
				? "All Claude Agent SDK accounts are blocked until re-login."
				: `All Claude Agent SDK accounts are blocked until ${new Date(soonestUnblockAt).toISOString()}.`,
		);
