...
import { CLAUDE_AGENT_SDK_PROVIDER_ID, pinProviderAccount, removeProviderAccount } from "./account-management.ts";
import { type AccountSlot, type ClaudeAgentSdkCredential, emptyCredential, listAccounts } from "./accounts.ts";
import { selectAccount } from "./affinity.ts";
import { type ClaudeAgentSdkProviderSettings, loadClaudeAgentSdkProviderSettingsFromDisk } from "./settings.ts";
...
export interface ClaudeAccountCommandDeps {
	loadSettings?: (cwd: string) => ClaudeAgentSdkProviderSettings;
	environment?: CommandEnvironment;
}

function asCredential(value: Credential | undefined): ClaudeAgentSdkCredential | undefined {
	return value?.type === "oauth" ? (value as ClaudeAgentSdkCredential) : undefined;
}
...
function readAccounts(
	...
	credential: ClaudeAgentSdkCredential | undefined;
	accounts: AccountSlot[];
} {
	const credential = asCredential(ctx.modelRegistry.authStorage.get(CLAUDE_AGENT_SDK_PROVIDER_ID));
	return {
		credential,
...
function accountError(ctx: ExtensionCommandContext, name: string): void {
	ctx.ui.notify(`Claude Agent SDK account '${name}' does not exist.`, "error");
}
...
function authEventMessage(event: unknown): string {
	if (event === null || typeof event !== "object") return "Claude Agent SDK authentication update.";
	const value = event as Record<string, unknown>;
	if (value.type === "auth_url" && typeof value.url === "string") {
		return `Open this URL to authorize Claude Agent SDK:\n${value.url}`;
	}
	if (value.type === "device_code" && typeof value.verificationUri === "string") {
		return `Open this URL to authorize Claude Agent SDK:\n${value.verificationUri}`;
	}
	return typeof value.message === "string" ? value.message : "Claude Agent SDK authentication update.";
}
...
export function registerClaudeAccountCommand(pi: ExtensionAPI, deps: ClaudeAccountCommandDeps = {}): void {
	const loadSettings = deps.loadSettings ?? loadClaudeAgentSdkProviderSettingsFromDisk;
	const environment = deps.environment ?? ((name: string) => process.env[name]);

	pi.registerFlag("claude-account", {
		type: "string",
		description: "Pin Claude Agent SDK account for this session.",
	});
	...
	pi.registerCommand("claude-account", {
		description: "List and manage Claude Agent SDK accounts.",
...
function showAccounts(
	ctx: ExtensionCommandContext,
	settings: ClaudeAgentSdkProviderSettings,
	environment: CommandEnvironment,
): void {
	...
	const lines = ["Claude Agent SDK accounts:"];
...
async function addAccount(ctx: ExtensionCommandContext): Promise<void> {
	...
	try {
		await ctx.modelRegistry.modelRuntime.login(CLAUDE_AGENT_SDK_PROVIDER_ID, "oauth", {
			signal: ctx.signal,
			prompt: async (prompt) => {
		...
		ctx.ui.notify("Claude Agent SDK account added.", "info");
	} catch (error) {
		const message = error instanceof Error ? error.message : String(error);
		if (message !== "Login cancelled") ctx.ui.notify(`Failed to add Claude Agent SDK account: ${message}`, "error");
	}
}
...
async function removeNamedAccount(
	...
	if (target.source === "env") {
		ctx.ui.notify(`Claude Agent SDK account '${name}' comes from the environment and cannot be removed.`, "error");
		return;
	}
	try {
		await removeProviderAccount(ctx.modelRegistry.authStorage, CLAUDE_AGENT_SDK_PROVIDER_ID, name);
		ctx.ui.notify(`Removed Claude Agent SDK account: ${name}.`, "info");
	} catch (error) {
...
async function pinNamedAccount(
	...
	try {
		await pinProviderAccount(ctx.modelRegistry.authStorage, CLAUDE_AGENT_SDK_PROVIDER_ID, name);
		ctx.ui.notify(`Pinned Claude Agent SDK account: ${name}.`, "info");
	} catch (error) {
		ctx.ui.notify(error instanceof Error ? error.message : String(error), "error");
	}
}
...
async function unpinAccount(ctx: ExtensionCommandContext): Promise<void> {
	...
	if (!credential?.pinned) {
		ctx.ui.notify("No stored Claude Agent SDK account pin is set.", "info");
		return;
	}
	await pinProviderAccount(ctx.modelRegistry.authStorage, CLAUDE_AGENT_SDK_PROVIDER_ID, null);
	ctx.ui.notify("Unpinned Claude Agent SDK account.", "info");
}
