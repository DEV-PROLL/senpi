...
- Rendered static prompt: 11,435 → 13,521 chars (+2,086, +18.2%, ~+520 tokens) under the same fixed options.
...
- Growth is defended per the entropy gate: every added directive replaces or absorbs weaker text (two overlapping fallback rules merged into one, three sentences deleted outright), and the preceding diet in this same file freed 1,213 chars, so the net against the pre-diet prompt is +873.
