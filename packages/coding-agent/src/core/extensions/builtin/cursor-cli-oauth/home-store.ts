...
function accountPaths(
  ...
  if (!isInside(accountsRoot, slotRoot) || !isInside(accountsRoot, home)) {
    throw new Error(
      `Cursor account HOME for '${accountName}' escapes the accounts root`,
    );
  }
  const cursorDirectory = resolve(home, ".cursor");
  if (!isInside(home, cursorDirectory)) {
    throw new Error(
      `Cursor credential directory for '${accountName}' escapes its HOME`,
    );
  }
...
function parseCredential(contents: string): CursorFileCredential {
  ...
  if (
    ...
    throw new Error(
      "Invalid Cursor credential: auth.json does not match the file-store contract",
    );
  }
  return {
...
function readBackSlot(
  ...
  return credential.refreshToken === slot.refresh
    ? slot
    : { ...slot, refresh: credential.refreshToken };
}
