...
import {
  ...
  withRemoteCatalog,
} from "./remote-catalog-provider.ts";
...
export type CredentialRotationStreamOptions = StreamOptions &
  ModelsRequestTransforms & { affinityKey?: string };
 ...
 */
function mightHoldEnvCredentialPool(
  providerId: string,
  env: (name: string) => string | undefined,
): boolean {
  const envVars = getApiKeyEnvVars(providerId);
  const primary =
    envVars?.find((name) => name.endsWith("_API_KEY")) ?? envVars?.[0];
...
function mightHoldCredentialPool(
  ...
  if (credential) {
    const accounts = Object.entries(credential).find(
      ([key]) => key === "accounts",
    )?.[1];
...
function withPayloadRequestMetadata(
...
export class ModelRuntime implements Models {
  ...
  private credentialPoolModules:
    | Promise<{
        ...
        repository: InstanceType<
          typeof import("./credential-pool/state-store.ts").CredentialSlotRepository
        >;
      }>
    | undefined;
  private constructor(
  ...
  static async create(
    ...
    const builtinModelDataGeneratedAt =
      builtinProviderCatalog.getBuiltinModelDataGeneratedAt();
    const providers = builtinProviderCatalog
      .builtinProviders()
      .map((provider) =>
        provider.refreshModels ||
        !remoteCatalogServesProvider(provider.id, options.catalogBaseUrl)
          ? provider
          : withRemoteCatalog(
              provider,
    ...
    const controller =
      refreshFromNetwork && options.modelRefreshTimeoutMs !== undefined
        ? new AbortController()
        : undefined;
    const timeout = controller
      ? setTimeout(() => controller.abort(), options.modelRefreshTimeoutMs)
      : undefined;
  ...
  private recomposeProvider(providerId: string): void {
    ...
    try {
      this.models.setProvider(
        composeModelProvider(providerId, base, this.config, extension),
      );
      this.compositionErrors.delete(providerId);
    } catch (error) {
  ...
  private queueAvailabilityRefresh(signal?: AbortSignal): Promise<void> {
    ...
    return this.runAvailabilityRefresh(seq, errorSeq, effectiveSignal).catch(
      (error) => {
        if (
  ...
  private async refreshProviderAvailability(
    ...
    try {
      ...
      const availableById = new Map(
        [
          ...this.snapshot.available.filter(
      ...
      this.snapshot = {
        all,
        available: all.flatMap(
          (model) => availableById.get(`${model.provider}\0${model.id}`) ?? [],
        ),
        configuredProviders,
        storedProviders,
        auth: authByProvider,
  ...
  isUsingSubscription(providerId: string): boolean {
    return (
      this.isUsingOAuth(providerId) &&
      this.models.getProvider(providerId)?.auth.oauth?.isSubscription === true
    );
  }
  ...
  private enqueueCredentialOperation<T>(
    providerId: string,
    signal: AbortSignal,
    task: () => Promise<T>,
  ): Promise<T> {
    const previous =
      this.credentialOperations.get(providerId) ?? Promise.resolve();
  ...
  getProviderAuthStatus(providerId: string): AuthStatus {
    ...
    return check
      ? { configured: true, source: "environment", label: check.source }
      : { configured: false };
  }
  ...
  >(
    ...
    const resolution = await this.getAuth(model, {
      ...
      ...(slotAuth?.slotName === undefined
        ? {}
        : { slotName: slotAuth.slotName }),
    });
    if (!resolution)
    ...
    const compatibility = this.getCompatibilityRequestConfig(
      model,
      resolution.env,
    );
    const extraBody =
    ...
    return {
      ...
      options: {
        ...providerOptions,
        apiKey:
          slotAuth?.apiKey ?? providerOptions.apiKey ?? resolution.auth.apiKey,
        headers,
        extraBody,
        env,
   ...
   */
  private couldRotateCredentials(
    model: Model<Api>,
    ...
    if (
      this.config.getProvider(model.provider)?.credentials?.rotation === false
    )
      return false;
  ...
  private async credentialRotationSources(
    ...
    if (!mightHoldCredentialPool(model.provider, credential, env))
      return undefined;
  ...
  stream<TApi extends Api>(
    ...
    return lazyStream(model, async () => {
      const streamOptions = options as
        CredentialRotationStreamOptions | undefined;
  ...
  async fetchDeferred(
    ...
    return lazyStream(model, async () => {
      ...
      return prepared.provider.fetchDeferred(
        prepared.model,
        handle,
        prepared.options as DeferredFetchOptions,
      );
    }).result();
  }
  ...
  async cancelDeferred(
    ...
    await prepared.provider.cancelDeferred(
      prepared.model,
      handle,
      prepared.options as DeferredCancelOptions,
    );
  }
  ...
  async refresh(
    ...
    const result = ((await this.models.refresh(refreshOptions)) as
      ModelsRefreshResult | undefined) ?? {
  ...
  registerNativeProvider(
    ...
    if (composedOAuth)
      this.credentials.registerOAuthProvider(provider.id, composedOAuth);
  ...
  registerProvider(
    ...
    if (composedOAuth)
      this.credentials.registerOAuthProvider(providerId, composedOAuth);
    ...
    if (
      ...
      this.snapshot = {
        ...
        auth,
        configuredProviders,
        available: this.snapshot.all.filter((model) =>
