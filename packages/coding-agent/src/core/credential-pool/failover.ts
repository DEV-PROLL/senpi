...
export class CredentialFailoverError extends Error {
  ...
  constructor(
    ...
    original: unknown,
    options: { suppressTurnRetry: boolean; retryAt?: number },
  ) {
    const detail =
      original instanceof Error ? original.message : String(original);
    super(
      `${options.suppressTurnRetry ? TURN_RETRY_SUPPRESSION_PREFIX : ""}${detail}`,
      { cause: original },
    );
...
export async function* runCredentialFailover<TEvent, TSlot extends RunSlot>(
  ...
  while (true) {
    ...
    const candidates = slots.filter(
      (slot) => !attempted.has(slot.name) && isAvailable(slot, now()),
    );
    ...
    try {
      ...
      const failureCount =
        (slot.failureCount ?? 0) + (retriesBySlot.get(slot.name) ?? 0);
