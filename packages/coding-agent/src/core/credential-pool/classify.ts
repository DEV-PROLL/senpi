import {
  DEFAULT_SLOT_BLOCK_MS,
  MAX_SLOT_BLOCK_MS,
} from "@earendil-works/pi-ai/auth/pool/failover";
...
export function rateLimitCooldown(
  ...
  const floored =
    serverHintMs === undefined ? backoff : Math.max(backoff, serverHintMs);
  return {
...
const INVALID_KEY_TEXT =
  /invalid[ _-]?(?:api[ _-]?)?key|authentication[_ ]?error|invalid x-api-key|unauthorized/i;
const ACCOUNT_SCOPED_403_TEXT =
  /account|credential|token|api[ _-]?key|organization|subscription/i;
...
export function classifyCredentialFailure(
  ...
  const text = normalized.messageCarriesBody
    ? normalized.message
