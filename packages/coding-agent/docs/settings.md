...
| `retry.billingErrorPolicy` | `"fallback"` \| `"swap"` | `"fallback"` | Billing-class failures (credit/quota exhaustion): temporary revertable fallback or permanent session model swap |
...
Billing-class failures — Anthropic's 400 *credit balance is too low*, OpenAI's 429 `insufficient_quota`, and other credit/quota exhaustion responses — never recover by retrying the same account. With the default `retry.billingErrorPolicy: "fallback"` they take the ordinary unpinned hard-error switch and revert after the 30-minute billing cooldown. Set it to `"swap"` to pin the chain candidate as the session model: the switch never auto-reverts, exactly like a refusal-pinned fallback, so later turns keep running on the replacement model instead of returning to the exhausted account.
